...
import 'package:zems/data/model/place_suggetion.dart';
import 'package:zems/data/service/api/maps_place.dart';
  ...
  Future<void> getNearestHospitals({double? radius}) async {
    try {
      ...
      if (hospitalsData.isEmpty) {
        emit(FindHospitalFailure(message: 'No medical facilities found nearby. Please try a different location.'));
        return;
      }

      emit(FindHospitalSuccess(hospitalsList: hospitalsData));
    } on DioException catch (e) {
      log('Dio error in getNearestHospitals: ${e.message}');
      emit(FindHospitalFailure(message: 'Network error. Please check your connection and try again.'));
    } catch (err) {
      log('Error in getNearestHospitals: $err');
      emit(FindHospitalFailure(message: 'Failed to load medical facilities. Please try again.'));
    }
  }
}
