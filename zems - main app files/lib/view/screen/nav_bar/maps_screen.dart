...
import 'package:zems/utils/helper/location.dart';
import 'package:zems/data/model/place_directions.dart';
import 'package:zems/data/model/place_location.dart';
...
import 'package:zems/view/widget/custom_tooltip.dart';
import 'package:zems/view/widget/directions_details_card.dart';
import 'package:zems/view/widget/floating_search_bar.dart';
...
class _MapScreenState extends State<MapScreen> {
  ...
  static final CameraPosition _myCurrrentPositionCameraPosition =
      CameraPosition(
          bearing: 0,
          target: LatLng(_position?.latitude ?? -19.0154, _position?.longitude ?? 29.1549),
          tilt: 0.0,
          zoom: 6);
  ...
  void _addDefaultMedicalFacility() {
    // Add Belvedere Medical Centre as a default marker
    final defaultMarker = Marker(
      markerId: const MarkerId('belvedere_medical_centre'),
      position: const LatLng(-17.828483, 30.9387013),
      infoWindow: const InfoWindow(
        title: 'Belvedere Medical Centre (BMC)',
        snippet: 'Phone: 08677186798',
      ),
    ...
    return BlocConsumer<MapsCubit, MapsState>(
      listener: (context, state) {
        if (state is FindHospitalLoading) {
          ...
          customSnackBar(
              context, 'Error, Check Your Connection.');
        }
      },
