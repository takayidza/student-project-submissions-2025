am4core.ready(function() {
    ...
    // Create chart
    var chart = am4core.create("last24_wateruse_chart", am4charts.XYChart);
    ...
    fetch("/api/last24_water_usage/")
        .then(response => response.json())
        .then(data => {
            // Prepare data for chart
            chart.data = data.chart_data.map(entry => ({
                timestamp: entry.timestamp,
                water_usage: Math.abs(entry.water_usage),  // show as positive
                usage_type: entry.water_usage < 0 ? "usage" : "refill",  // logic based on ORIGINAL sign
                water_level: entry.level
            }));            
        });
    ...
    categoryAxis.title.text = "Time (HH:MM)";
    ...
    // 🎨 Color bars based on positive or negative usage
    usageSeries.columns.template.adapter.add("fill", function(fill, target) {
        const type = target.dataItem.dataContext.usage_type;
        return type === "usage" ? am4core.color("#ff4d4d") : am4core.color("#13afb2");
    });

    usageSeries.columns.template.adapter.add("tooltipText", function(text, target) {
        const type = target.dataItem.dataContext.usage_type;
        const label = type === "usage" ? "Water Used" : "Refill";
        return `${label}: [bold]{valueY}[/]`;
    });

    usageSeries.columns.template.adapter.add("tooltipBackground.fill", function(fill, target) {
        const data = target.dataItem?.dataContext;
        if (!data) return fill;
    
        return data.usage_type === "usage"
            ? am4core.color("#fe7383")  // Red-ish for usage
            : am4core.color("#13afb2"); // Teal for refill
    });
